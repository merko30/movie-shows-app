...
import customRender, { initialValue as value, history } from '../utils/customRender';
...
describe('Detail', () => {
  ...
  test('should show video if movie/show has video, otherwise show poster', async () => {
    const { getByTestId, queryByAltText } = customRender(<Detail {...routeComponentPropsMock} />, {
      value: {
        ...value,
        state: {
          ...value.state,
  ...
  test('click on back icon should navigate back to home', async () => {
    ...
    const { getByTestId, history: routeHistory } = customRender(
    ...
    expect(routeHistory.location.pathname).toBe(`/${type}/${id}`);
